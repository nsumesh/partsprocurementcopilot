...
**What it does:** React entry point. Mounts `<App />` into `#root` wrapped in `StrictMode` and `ErrorBoundary` (uncaught render errors show a reload card instead of a white screen), and imports `index.css` (Tailwind directives).
...
**What it does:** Results screen that fires on mount, streams parts via `streamSearch` (forwarding `urgency_deadline` from router state in the request), and applies `applyFilters` client-side to the received `SearchResultPart[]` array. Incoming parts are inserted rank-sorted by their SSE `index` so out-of-order fitment completions don't scramble the display order. Manages all result-flow state: `results`, `isStreaming`, `clarifyQuestion`, `searchError`, `selectedResult`, `confirmTarget`, `filters`, `procureTarget`, `selectedVendorPart`, `procurementJob`, `procureDeadline`, and `procureError`. `LocationState` includes `urgency_deadline: string | null`. Redirects to `/` if accessed without router state. Header includes navigation links for "Orders" and "Procurement".

Procurement flow: `PartDetail` "Procure →" sets `procureTarget` and `procureDeadline` → `VendorSelector` modal opens (fetches vendors for the part), on vendor select calls `createProcurementJob` — failures are caught and surfaced as an inline `procureError` message instead of dying silently → `OutreachConfirm` modal opens → user reviews/edits the Haiku-generated email, `sendOutreach` sends it → navigates to `/procurement` and clears all procure state.
